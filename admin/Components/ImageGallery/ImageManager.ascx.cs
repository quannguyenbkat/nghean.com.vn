...
public partial class admin_Components_ImageGallery_ImageManager : BaseUserControl
{
    ...
    {
        txtName.Attributes.Add("onkeypress", "return clickButton1(event,'" + btnSearch.ClientID + "')");
        if (!IsPostBack)
        {
            ImageTypeBiz.BuildImageTypeTree(ddlImageTypeID);
            ddlImageTypeID.SelectedIndex = BicSession.ToInt32("SelectCategoryImage");
            hdfLangManager.Value = BicHtml.GetRequestString("l") == "" ? BicXML.ToString("DefaultLanguageAdmin", "SearchEngine") : BicHtml.GetRequestString("l");
    ...
    public void BinddingImageManager()
    {
        ...
        if (ddlImageTypeID.SelectedValue != "0")
            data.Conditioning.Add(new ConditioningItem(ImageEntity.FIELD_IMAGETYPEID, ddlImageTypeID.SelectedValue, Operator.EQUAL, CompareType.NUMERIC));
        ...
        //if (!BicMemberShip.CurrentUserName.Equals("administrator") && !Roles.IsUserInRole(BicMemberShip.CurrentUserName, "Administrator"))
            if (!Roles.IsUserInRole(BicMemberShip.CurrentUserName, "Administrator"))
        {
            data.Conditioning.Add(new ConditioningItem(ImageEntity.FIELD_USERNAME, BicMemberShip.CurrentUserName, Operator.EQUAL, CompareType.STRING));
        }
        ...
        if (txtName.Text != string.Empty)
            lblMessage.Text = string.Format("<b>Tìm được: </b><b> {0} ảnh</b> với tên <b>\"{1}\"</b>", data.TotalItems, BicConvert.ToString(txtName.Text));
        else
    ...
    {
        BicSession.SetValue("SelectedIndexImage","0");
        BinddingImageManager();
        //Pager1.PageIndex = 0;
    }
    ...
    protected void dlImage_ItemDataBound(object sender, ListViewItemEventArgs e)
    {
        ...
        string path = !string.IsNullOrEmpty(BicConvert.ToString(drv[ImageEntity.FIELD_NAME])) ? (BicApplication.URLPath(BicConvert.ToString(drv[ImageEntity.FIELD_PATH])) + BicConvert.ToString(drv[ImageEntity.FIELD_NAME])) : BicConvert.ToString(drv[ImageEntity.FIELD_PATH]);
        ...
        myHtmlImage.Attributes.Add("ondblclick", string.Format("return ImageClick('{0}','{1}','{2}','{3}');", id, width, height, path));
        //Thiết lập sự kiện xem ảnh lớn
        fullExpand.HRef = path;
        fullExpand.Attributes.Add("onclick", "return hs.expand(this,{ wrapperClassName: 'highslide-no-border',align: 'center' })");
