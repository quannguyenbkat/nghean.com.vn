    ...
    protected void Save_Click(object sender, EventArgs e)
    {
        ...
        if (_imageEntity != null)
        {
            ...
            if (!File.Exists(currentImage.FullName))
            {
                ...
                ScriptManager.RegisterStartupScript(Page, Page.GetType(), "noimageedited", "ShowImage('" + originalImageUrl + "');" + "alert('Bạn cần click Xem trước khi Lưu thay đổi.');", true);
                return;
            }
            ...
            if (size.Length == 2)
            {
                BicImage.CreateThumbs(MapPath("~/FileUpload/Images"), _imageEntity.Name, MapPath("~/FileUpload/Images/thumb"), BicConvert.ToInt32(txtImgWidth.Value), BicConvert.ToInt32(txtImgHeight.Value), BicConvert.ToInt32(size[0]), BicConvert.ToInt32(size[1]));
            }
