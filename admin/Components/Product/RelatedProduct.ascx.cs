...
public partial class admin_Components_Product_RelatedProduct : UserControl
{
    ...
    public string RelatedProductId
    {
        get
        {
            var obj = ViewState["RelatedProductId"];
            return string.IsNullOrEmpty(Convert.ToString(obj)) ? string.Empty : (string)obj;
        }
    ...
    private DataTable LoadProduct()
    {
        ...
        bicData.Conditioning.Add(new ConditioningItem { Query = "ProductId in (" + RelatedProductId + ")", TypeOfCondition = TypeOfCondition.QUERY });
        bicData.Sorting.Add(new SortingItem("(CharIndex(',' + CONVERT(NVARCHAR,ProductId) +',','," + RelatedProductId + ",'))", false));
    ...
    public void GetValue()
    {
        ...
        {
            lvRelatedProduct.QueryCondition = string.Format("LanguageKey = '{0}' and Title like N'%{1}%'", Lang, txtFilter.Text.Trim());
        }
        else
        {
    ...
    protected void chkRelated_OnCheckedChanged(object sender, EventArgs e)
    {
        ...
        RelatedProductId = relatedProductIds.ToArray().Aggregate(string.Empty, (current, productId) => current + ((current == string.Empty ? "" : ",") + productId));
        lvProduct.DataSource = RelatedList;
        lvProduct.DataBind();
