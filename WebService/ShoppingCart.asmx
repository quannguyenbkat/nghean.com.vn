                                              ...
                                              string.IsNullOrEmpty(productEntity.Price)
                                                  ...
                                                  : ((Convert.ToDecimal(
                                                      ...
                                                      (string.IsNullOrEmpty(productEntity.SaleOff)
                                                           ? Convert.ToDecimal(menuEntity.MenuIcon)
            ...
            if (productEntity.OutOfStock)
            {
                ...
                var product = new ProductCart
                {
                    ...
                    ProductVat =
                        string.IsNullOrEmpty(productEntity.Price)
                            ? "0"
                            : ((Convert.ToDecimal(productEntity.Price.Replace(" USD", "").Replace(" VND", "").Replace(" VNĐ", "")) * (string.IsNullOrEmpty(productEntity.SaleOff) ? Convert.ToDecimal(menuEntity.MenuIcon) : Convert.ToDecimal(productEntity.SaleOff))) / 100 * numberProduct).ToString(),
                    //ProductVat = "0",
                    ImageId = productEntity.ImageID,
                    Code = productEntity.Code,
                    TotalFullVat =
