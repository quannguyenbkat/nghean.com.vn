$(document).ready(function () {
    ...
    $(".cagree .agree,.cagree .disagree").live("click", function () {
        ...
        var b = $(this).parent();
        $.ajax({
            url: getBaseURL() + 'WebService/CommentReview.ashx?id=' + $(this).attr("value") + '&vote=' + a + '&aid=' + $(this).parent().children('.comment').attr("value"),
...
function GetComments(a, b, c) {
    $.ajax({
        url: getBaseURL() + 'WebService/Comment.ashx?id=' + a,
        success: function (response) {
            b.html(response);
